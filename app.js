(() => {
  ...
  const DEFAULT_FILTERS = Object.freeze({ keyword: '', system: '', favorite: '', count: '', time: '', lost: '', trend: '', participation: '', combat: '', sort: 'updatedAt-desc' });
  const state = { route: {}, scenarios: [], drafts: [], settings: { theme: 'light', confirmDelete: true, promptBackup: false, discordFormat: defaultDiscordFormat() }, filters: { ...DEFAULT_FILTERS }, editImages: [], editCharacters: [], editSessions: [], editHoItems: [{ type: '', content: '' }], editHoType: '', activeDraftId: null, pendingScenarioTransition: null, mobileMenuOpen: false };
  let db;
  ...
  function normalizeScenario(scenario, index = 0) {
    ...
    const legacyPersonal = isObject(scenario.personal) ? scenario.personal : normalized.personal;
    const isKp = legacyPersonal.isKp !== undefined ? Boolean(legacyPersonal.isKp) : legacyPersonal.kp?.isKp !== undefined ? Boolean(legacyPersonal.kp.isKp) : legacyPersonal.kp?.status === MASTER.kpStatus[1] || legacyPersonal.kpStatus === MASTER.kpStatus[1];
    const isPl = legacyPersonal.isPl !== undefined ? Boolean(legacyPersonal.isPl) : legacyPersonal.pl?.isPl !== undefined ? Boolean(legacyPersonal.pl.isPl) : legacyPersonal.pl?.status === MASTER.playStatus[1] || legacyPersonal.playStatus === MASTER.playStatus[1];
    normalized.personal.isKp = isKp;
    normalized.personal.isPl = isPl;
    normalized.personal.kp = { url: legacyPersonal.kp?.url ?? legacyPersonal.url ?? '', status: isKp ? MASTER.kpStatus[1] : MASTER.kpStatus[0], isKp, memo: legacyPersonal.kp?.memo ?? '' };
    const legacyCharacters = array(legacyPersonal.pl?.characters).map(character => ({ name: character?.name || '', playerName: character?.playerName || '', iacharaUrl: character?.iacharaUrl || character?.url || '', image: typeof character?.image === 'string' ? { src: character.image } : (character?.image || null), ho: character?.ho || '' }));
    normalized.personal.pl = { status: isPl ? MASTER.playStatus[1] : MASTER.playStatus[0], isPl, memo: legacyPersonal.pl?.memo ?? '', characters: legacyCharacters };
    normalized.personal.sessions = array(legacyPersonal.sessions).map((session, index) => ({ name: session?.name || `${index + 1}陣`, role: session?.role || 'PL', characters: array(session?.characters).map(character => ({ name: character?.name || '', playerName: character?.playerName || '', iacharaUrl: character?.iacharaUrl || character?.url || '', image: typeof character?.image === 'string' ? { src: character.image } : (character?.image || null), ho: character?.ho || '' })) }));
    if (!normalized.personal.sessions.length && legacyCharacters.length) normalized.personal.sessions = [{ name: '1陣', role: 'PL', characters: legacyCharacters }];
    normalized.personal.url = normalized.personal.kp.url;
    normalized.personal.kpStatus = normalized.personal.kp.status;
    normalized.personal.playStatus = normalized.personal.pl.status;
  ...
  function filteredScenarios() {
    ...
    let result = state.scenarios.filter(s => {
      const b = s.basic || {}, sc = s.scenario || {}, p = s.personal || {};
      const keyword = f.keyword.trim().toLowerCase();
      if (keyword && !scenarioText(s).includes(keyword)) return false;
      if (f.system && b.system !== f.system) return false;
      if (f.version && b.version !== f.version) return false;
      if (f.favorite === 'favorite' && !s.favorite) return false;
      if (f.lost && sc.lostRate !== f.lost) return false;
      if (f.trend && !array(sc.trends).includes(f.trend)) return false;
      if (f.participation && participationKey(p) !== f.participation) return false;
      if (f.combat && sc.combat !== f.combat) return false;
      if (f.count && !personLabel(b).includes(f.count)) return false;
      if (f.time && !matchesTimeFilter(b, f.time)) return false;
      return true;
    });
    const [key, dir] = f.sort.split('-');
    result.sort((a, b) => {
      let av = key === 'title' ? a.basic.title : key === 'author' ? a.basic.author : a[key];
      let bv = key === 'title' ? b.basic.title : key === 'author' ? b.basic.author : b[key];
      av = av || '';
      bv = bv || '';
      const cmp = key === 'title'
        ? compareTitles(a.basic.title, b.basic.title, a.basic.titleReading, b.basic.titleReading)
        : key === 'author'
          ? compareReading(a.basic.author, b.basic.author, a.basic.authorReading, b.basic.authorReading)
          : typeof av === 'string' ? av.localeCompare(bv, 'ja') : av - bv;
      return dir === 'asc' ? cmp : -cmp;
    });
    return result;
  }
  function participationKey(personal = {}) { const kp = Boolean(personal.isKp), pl = Boolean(personal.isPl); return kp && pl ? 'both' : kp ? 'kp-only' : pl ? 'pl-only' : 'none'; }
  function participationLabel(personal = {}) { return ({ 'kp-only': 'KP', 'pl-only': 'PL', both: 'KP・PL', none: 'なし' })[participationKey(personal)]; }
  function resetFilters() { Object.assign(state.filters, DEFAULT_FILTERS); }
  ...
  function bindList() { document.querySelectorAll('.scenario-card').forEach(el => { el.addEventListener('click', () => { const image = el.querySelector('img.card-image') || el.querySelector('.card-image'); const rect = image?.getBoundingClientRect(); state.pendingScenarioTransition = rect ? { id: el.dataset.id, rect: { left: rect.left, top: rect.top, width: rect.width, height: rect.height } } : null; go('detail', el.dataset.id); }); el.querySelector('[data-action="favorite"]')?.addEventListener('click', e => { e.stopPropagation(); toggleFavorite(el.dataset.id); }); }); const ids = ['filter-system', 'filter-version', 'filter-count', 'filter-time', 'filter-lost', 'filter-trend', 'filter-participation', 'filter-combat', 'filter-sort']; ids.forEach(id => document.querySelector(`#${id}`)?.addEventListener('change', e => { const map = { 'filter-system': 'system', 'filter-version': 'version', 'filter-count': 'count', 'filter-time': 'time', 'filter-lost': 'lost', 'filter-trend': 'trend', 'filter-participation': 'participation', 'filter-combat': 'combat', 'filter-sort': 'sort' }; state.filters[map[id]] = e.target.value; if (id === 'filter-system') { state.filters.version = ''; renderList(); } else renderList(); })); document.querySelector('[data-action="advanced"]')?.addEventListener('click', () => document.querySelector('#advanced-filters')?.classList.toggle('open')); document.querySelectorAll('[data-action="reset"]').forEach(el => el.addEventListener('click', () => { resetFilters(); renderList(); })); }
  ...
  function defaultDiscordFormat() {
    return { titleTemplate: '**{system}　{title}**', codeBlock: true, showLabels: true, sections: [
      ...
      author: { enabled: false, label: '作者名' }, url: { enabled: false, label: '購入・配布URL' }, kp: { enabled: false, label: 'KP' },
      pl: { enabled: false, label: 'PL' }, memo: { enabled: false, label: '自由メモ' }
    } };
  }
  ...
  function blankData() { return { favorite: false, basic: { title: '', titleReading: '', system: '', version: '', author: '', authorReading: '', scenarioType: 'normal', countType: 'fixed', fixedCount: '', minCount: '', maxCount: '', freeCount: '', timeType: 'fixed', fixedTime: '', fixedTimeValue: '', fixedTimeUnit: '時間', minTime: '', minTimeValue: '', minTimeUnit: '時間', maxTime: '', maxTimeValue: '', maxTimeUnit: '時間', freeTime: '', stage: '' }, campaign: { episodeCount: 0, episodes: [] }, scenario: { recommendedSkills: '', secondarySkills: '', notRecommended: '', lostRate: '', lostRateNote: '', hoType: '', hoContent: '', trends: [], combat: '', notes: '' }, trailer: { text: '', images: [] }, personal: { url: '', isKp: false, isPl: false, kpStatus: '', playStatus: '', memo: '', kp: { url: '', status: '', isKp: false, memo: '' }, pl: { status: '', isPl: false, memo: '', characters: [] }, sessions: [] } }; }
  function roleCheckboxes(isKp, isPl) { return `<div class="field"><label>参加区分</label><div class="check-grid"><label class="check-label"><input type="checkbox" name="isKp" ${isKp ? 'checked' : ''}>KP</label><label class="check-label"><input type="checkbox" name="isPl" ${isPl ? 'checked' : ''}>PL</label></div></div>`; }
  ...
  function renderDiscordFormat() {
    const format = normalizeDiscordFormat(state.settings.discordFormat);
    const fieldLabels = [['system', 'システム'], ['count', '人数'], ['stage', '舞台'], ['recommendedSkills', '推奨技能'], ['secondarySkills', '準推奨技能'], ['combat', '戦闘'], ['lostRate', 'ロスト率'], ['lostRateNote', 'ロスト率補足'], ['time', 'プレイ時間'], ['trends', '傾向'], ['notRecommended', '非推奨技能'], ['notes', '注意事項'], ['author', '作者名'], ['url', '購入・配布URL'], ['kp', 'KP'], ['pl', 'PL'], ['memo', '自由メモ']];
  ...
  renderDetail = id => {
    ...
    const scenarioForPersonal = state.scenarios.find(item => item.id === id);
    const personalSections = [...document.querySelectorAll('.section-card')].filter(section => section.querySelector('.section-heading')?.textContent?.trim() === '個人管理');
    if (scenarioForPersonal && personalSections[0]) {
      const personal = scenarioForPersonal.personal || {};
      const kp = personal.kp || {};
      const pl = personal.pl || {};
      const characters = array(pl.characters);
      personalSections[0].querySelector('.section-heading').textContent = 'シナリオ管理';
      personalSections[0].querySelector('.data-grid').innerHTML = `<div class="data-column">${urlItem('購入・配布URL', kp.url)}${dataItem('KP', personal.isKp ? '✓' : '')}${dataItem('PL', personal.isPl ? '✓' : '')}</div><div class="data-column">${dataItem('KPメモ', kp.memo)}</div><div class="data-column">${dataItem('登録日', formatDateTime(scenarioForPersonal.createdAt))}${dataItem('更新日', formatDateTime(scenarioForPersonal.updatedAt))}</div>`;
      const sessions = array(personal.sessions);
      personalSections[0].insertAdjacentHTML('afterend', `<section class="section-card"><h2 class="section-heading">参加記録</h2><div class="session-list">${sessions.length ? sessions.map(session => `<article class="session-card"><h3>${display(session.name)} <span class="tag">${escapeHtml(session.role === 'KP' ? 'KP' : 'PL')}</span></h3><div class="character-list">${array(session.characters).length ? array(session.characters).map(character => `<article class="character-card">${character.image?.src ? `<img src="${escAttr(character.image.src)}" alt="${escAttr(character.name || '参加キャラクター')}の立ち絵">` : ''}<div><h4>${display(character.name)}</h4>${dataItem('PL名', character.playerName)}${urlItem('いあきゃら', character.iacharaUrl)}${dataItem('HO', character.ho)}</div></article>`).join('') : '<span class="muted">PC未登録</span>'}</div></article>`).join('') : '<span class="muted">参加記録未登録</span>'}</div></section>`);
    }
    const scenario = state.scenarios.find(item => item.id === id);
    if (!scenario) return;
  ...
  function readCharactersFromForm() { const list = document.querySelector('#character-editor-list'); if (!list) return []; return [...list.querySelectorAll('.character-edit-card')].map(card => { const index = Number(card.dataset.characterIndex); return { name: card.querySelector('[name="character-name"]')?.value.trim() || '', iacharaUrl: card.querySelector('[name="character-url"]')?.value.trim() || '', image: state.editCharacters[index]?.image || null, ho: card.querySelector('[name="character-ho"]')?.value.trim() || '', memo: card.querySelector('[name="character-memo"]')?.value.trim() || '' }; }).filter(character => character.name || character.iacharaUrl || character.image || character.ho || character.memo); }
  function setupCharacterFieldsLegacy() {
    const form = document.querySelector('#scenario-form');
    const oldSection = [...(form?.querySelectorAll('.section-card') || [])].find(section => section.querySelector('.section-heading')?.textContent?.trim() === '個人管理');
    if (!form || !oldSection || form.querySelector('#pl-management-fields')) return;
    const scenario = state.route.id && state.route.id !== 'new' ? detail(state.route.id) : null;
    const personal = scenario?.personal || blankData().personal;
    const kp = personal.kp || { url: personal.url || '', status: personal.kpStatus || '', memo: personal.memo || '' };
    const pl = personal.pl || { status: personal.playStatus || '', memo: '', characters: [] };
    const characterSource = state.activeDraftId ? state.editCharacters : array(pl.characters);
    state.editCharacters = characterSource.map(character => ({ ...character, image: character.image || null }));
    oldSection.querySelector('.section-heading').textContent = 'シナリオ管理';
    oldSection.innerHTML = `<h2 class="section-heading">シナリオ管理</h2><div class="form-grid"><div class="form-column">${inputField('購入・配布URL', 'url', kp.url, { type: 'url', placeholder: 'https://example.com' })}${roleCheckboxes(personal.isKp, personal.isPl)}</div><div class="form-column">${textareaField('KPメモ', 'memo', kp.memo, 'KP用のメモを入力してください。')}</div></div>`;
    const plSection = document.createElement('section');
    plSection.id = 'pl-management-fields';
    plSection.className = 'section-card';
    plSection.innerHTML = `<h2 class="section-heading">PL管理</h2><div class="form-grid"><div class="form-column">${textareaField('PLメモ', 'plMemo', pl.memo, 'PL用のメモを入力してください。')}</div><div class="form-column"><div class="field"><label>参加キャラクター</label><div id="character-editor-list"></div><button type="button" class="btn small soft" data-action="add-character">＋ キャラクターを追加</button></div></div></div>`;
    oldSection.insertAdjacentElement('afterend', plSection);
    const list = plSection.querySelector('#character-editor-list');
    const sync = () => { state.editCharacters = state.editCharacters.map((character, index) => ({ ...character, name: list.querySelectorAll('[name="character-name"]')[index]?.value || character.name || '', iacharaUrl: list.querySelectorAll('[name="character-url"]')[index]?.value || character.iacharaUrl || '', ho: list.querySelectorAll('[name="character-ho"]')[index]?.value || character.ho || '', memo: list.querySelectorAll('[name="character-memo"]')[index]?.value || character.memo || '' })); };
    const render = () => { list.innerHTML = state.editCharacters.length ? state.editCharacters.map((character, index) => `<article class="character-edit-card" data-character-index="${index}"><div class="character-edit-header"><strong>キャラクター${index + 1}</strong><button type="button" class="btn small danger" data-remove-character="${index}">削除</button></div>${character.image?.src ? `<img class="character-image-preview" src="${escAttr(character.image.src)}" alt="立ち絵プレビュー">` : ''}${inputField('キャラクター名', 'character-name', character.name, { placeholder: '例：探索者名' })}${inputField('いあきゃらURL', 'character-url', character.iacharaUrl, { type: 'url', placeholder: 'https://iachara.com/...' })}<div class="field"><label>立ち絵</label><input type="file" accept="image/png,image/jpeg,image/webp" data-character-image="${index}"></div>${inputField('HO', 'character-ho', character.ho)}${textareaField('キャラクターメモ', 'character-memo', character.memo, 'このキャラクターについてのメモ')}</article>`).join('') : '<p class="helper">参加キャラクターを追加できます。</p>'; list.querySelectorAll('[data-remove-character]').forEach(button => button.addEventListener('click', () => { sync(); state.editCharacters.splice(Number(button.dataset.removeCharacter), 1); render(); })); list.querySelectorAll('[data-character-image]').forEach(input => input.addEventListener('change', async event => { const files = [...(event.target.files || [])].filter(isTrailerImage); if (!files.length) return; state.editCharacters[Number(input.dataset.characterImage)].image = await fileToDataUrl(files[0]); render(); })); };
    plSection.querySelector('[data-action="add-character"]').addEventListener('click', () => { sync(); state.editCharacters.push({ name: '', iacharaUrl: '', image: null, ho: '', memo: '' }); render(); });
    render();
  }
  function readSessionsFromForm() {
    return [...document.querySelectorAll('.session-edit-card')].map((card, sessionIndex) => ({
      name: card.querySelector('[name="session-name"]')?.value.trim() || `${sessionIndex + 1}陣`,
      role: card.querySelector('[name="session-role"]')?.value || 'PL',
      characters: [...card.querySelectorAll('.session-character-edit')].map((characterCard, characterIndex) => ({
        name: characterCard.querySelector('[name="character-name"]')?.value.trim() || '',
        playerName: characterCard.querySelector('[name="player-name"]')?.value.trim() || '',
        iacharaUrl: characterCard.querySelector('[name="character-url"]')?.value.trim() || '',
        image: state.editSessions[sessionIndex]?.characters?.[characterIndex]?.image || null,
        ho: characterCard.querySelector('[name="character-ho"]')?.value.trim() || ''
      }))
    })).filter(session => session.name || session.characters.length);
  }
  function setupPersonalFields() {
    const form = document.querySelector('#scenario-form');
    const oldSection = [...(form?.querySelectorAll('.section-card') || [])].find(section => section.querySelector('.section-heading')?.textContent?.trim() === '個人管理');
    if (!form || !oldSection) return;
    const scenario = state.route.id && state.route.id !== 'new' ? detail(state.route.id) : null;
    const personal = scenario?.personal || blankData().personal;
    const kp = personal.kp || { url: personal.url || '', status: personal.kpStatus || '', memo: personal.memo || '' };
    const sessions = state.activeDraftId ? state.editSessions : array(personal.sessions);
    state.editSessions = sessions.map(session => ({ name: session.name || '1陣', role: session.role || 'PL', characters: array(session.characters).map(character => ({ ...character, image: character.image || null })) }));
    oldSection.querySelector('.section-heading').textContent = 'シナリオ管理';
    oldSection.innerHTML = `<h2 class="section-heading">シナリオ管理</h2><div class="form-grid"><div class="form-column">${inputField('購入・配布URL', 'url', kp.url, { type: 'url', placeholder: 'https://example.com' })}${roleCheckboxes(personal.isKp, personal.isPl)}</div><div class="form-column">${textareaField('KPメモ', 'memo', kp.memo, 'KP用のメモを入力してください。')}</div></div>`;
    const plSection = document.querySelector('#pl-management-fields') || document.createElement('section');
    plSection.id = 'pl-management-fields';
    plSection.className = 'section-card';
    plSection.innerHTML = `<h2 class="section-heading">参加記録</h2><div id="session-editor-list"></div><button type="button" class="btn small soft" data-action="add-session">＋ 陣を追加</button>`;
    oldSection.insertAdjacentElement('afterend', plSection);
    const list = plSection.querySelector('#session-editor-list');
    const sync = () => { state.editSessions = readSessionsFromForm(); };
    const render = () => {
      list.innerHTML = state.editSessions.length ? state.editSessions.map((session, sessionIndex) => `<article class="session-edit-card" data-session-index="${sessionIndex}"><div class="session-edit-header"><strong>参加記録</strong><button type="button" class="btn small danger" data-remove-session="${sessionIndex}">削除</button></div><div class="session-edit-fields">${inputField('陣の名前', 'session-name', session.name, { placeholder: '例：1陣' })}${selectField('役割', 'session-role', [{ value: 'PL', label: 'PL' }, { value: 'KP', label: 'KP' }], session.role)}</div><div class="session-character-list">${session.characters.map((character, characterIndex) => `<div class="session-character-edit" data-character-index="${characterIndex}"><div class="character-edit-header"><strong>PC</strong><button type="button" class="btn small danger" data-remove-character="${characterIndex}">削除</button></div>${character.image?.src ? `<img class="character-image-preview" src="${escAttr(character.image.src)}" alt="立ち絵プレビュー">` : ''}${inputField('PC名', 'character-name', character.name)}${inputField('PL名', 'player-name', character.playerName)}${inputField('いあきゃらURL', 'character-url', character.iacharaUrl, { type: 'url', placeholder: 'https://iachara.com/...' })}<div class="field"><label>立ち絵</label><input type="file" accept="image/png,image/jpeg,image/webp" data-session-image="${sessionIndex}" data-character-image="${characterIndex}"></div>${inputField('HO', 'character-ho', character.ho)}</div>`).join('')}</div><button type="button" class="btn small soft" data-add-character="${sessionIndex}">＋ PCを追加</button></article>`).join('') : '<p class="helper">参加記録を追加できます。</p>';
      list.querySelectorAll('[data-remove-session]').forEach(button => button.addEventListener('click', () => { sync(); state.editSessions.splice(Number(button.dataset.removeSession), 1); render(); }));
      list.querySelectorAll('[data-add-character]').forEach(button => button.addEventListener('click', () => { sync(); const session = state.editSessions[Number(button.dataset.addCharacter)]; session?.characters.push({ name: '', playerName: '', iacharaUrl: '', image: null, ho: '' }); render(); }));
      list.querySelectorAll('[data-remove-character]').forEach(button => button.addEventListener('click', () => { sync(); const card = button.closest('.session-edit-card'); const session = state.editSessions[Number(card?.dataset.sessionIndex)]; session?.characters.splice(Number(button.dataset.removeCharacter), 1); render(); }));
      list.querySelectorAll('[data-session-image]').forEach(input => input.addEventListener('change', async event => { sync(); const session = state.editSessions[Number(input.dataset.sessionImage)]; const character = session?.characters[Number(input.dataset.characterImage)]; const file = [...(event.target.files || [])].find(isTrailerImage); if (character && file) { character.image = await fileToDataUrl(file); render(); } }));
    };
    plSection.querySelector('[data-action="add-session"]').addEventListener('click', () => { sync(); state.editSessions.push({ name: `${state.editSessions.length + 1}陣`, role: 'PL', characters: [] }); render(); });
    render();
  }
  ...
  readForm = () => {
    ...
    const plMemo = form?.elements.plMemo?.value.trim() || '';
    personal.kp = { url: personal.url || '', status: personal.isKp ? MASTER.kpStatus[1] : MASTER.kpStatus[0], isKp: personal.isKp, memo: personal.memo || '' };
    personal.pl = { status: personal.isPl ? MASTER.playStatus[1] : MASTER.playStatus[0], isPl: personal.isPl, memo: plMemo, characters: [] };
    personal.sessions = readSessionsFromForm();
    return data;
  };
  const baseBindEdit = bindEdit;
  bindEdit = (...args) => { baseBindEdit(...args); setupReadingFields(); setupPersonalFields(); document.querySelector('[data-action="cancel"]')?.addEventListener('click', () => { state.activeDraftId = null; }); };
})();
